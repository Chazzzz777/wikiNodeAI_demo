...
为了确保应用所有功能正常运行，您在飞书开放平台创建应用时，需要为其开通以下权限：

- `wiki:wiki:readonly`: 用于读取知识空间、知识库节点和文档元信息。
- `docx:document:readonly`: 用于获取飞书云文档（`doc` 和 `docx` 类型）的原始内容。

请在您的飞书应用配置的“权限管理”页面，确保以上两个权限已被添加并启用。
